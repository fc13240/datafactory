 ...
 * 2018/9/26
 */
public class Fy2DTask extends BaseTask {
    ...
    protected void config() {
        setTaskId("fy2d_task");
        setTaskName("风云2D数据分通道");
    }

    @Override
    protected void doWork(JobDataMap dataMap) {
        String statusFileName = JarToolUtil.getJarDir() + File.separator + "fy2d.txt";
        byte[] b = new byte[2];
        b[0] = 0x0d;
        b[1] = 0x0a;
        String c_string = new String(b);
        String sourceFolder = MainWindow.mainWindow.getFy2dSourceTextField().getText().trim();
        String destFolder = MainWindow.mainWindow.getFy2dDestTextField().getText().trim();
        //脚本位置
        ...
        if (srcDir.exists() && srcDir.isDirectory()) {
            ...
            BufferedReader br;
            try {
                br = new BufferedReader(new FileReader(JarToolUtil.getJarDir() + File.separator + "fy2d.txt"));
                String line;
                while (null != (line = br.readLine())) {
